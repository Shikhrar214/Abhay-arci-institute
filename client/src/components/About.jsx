  ...
  return (
    <>
      ...
      <motion.h1
        ...
        className="mt-24 md:mt-28 bg-gradient-to-r from-orange-500 to-yellow-500 
                   text-white text-center py-4 px-6 sm:px-10 md:px-24 lg:px-48 
                   rounded-full text-2xl sm:text-3xl md:text-4xl font-bold shadow-lg mx-4"
      >
        About Us
      </motion.h1>
      ...
      <motion.div
        ...
        className="my-8 p-4 sm:p-6 md:p-10 bg-gray-900 text-white flex flex-col 
                   md:flex-row items-center justify-center rounded-3xl mx-4 sm:mx-8 
                   shadow-lg"
      >
        <div className="flex flex-col md:flex-row items-center gap-6 sm:gap-8 lg:gap-10 max-w-6xl w-full text-center md:text-left">
          {/* Image */}
          <motion.div
            whileHover={{ scale: 1.05 }}
            transition={{ type: "spring", stiffness: 200 }}
            className="w-36 h-36 sm:w-44 sm:h-44 md:w-56 md:h-56 lg:w-64 lg:h-64 
                       bg-gradient-to-tr from-orange-400 to-orange-600 rounded-2xl 
                       flex items-center justify-center overflow-hidden shadow-md border-4 border-orange-300"
          >
            <img
          ...
          {/* Text */}
          <div className="flex flex-col gap-3 sm:gap-4 max-w-2xl px-2 sm:px-4 md:px-6 py-2">
            <motion.h2
              initial={{ opacity: 0, x: -50 }}
              whileInView={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6 }}
              className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-orange-400"
            >
            ...
            >
              As the Chairman of Aditya Rise Computer Institute, I'm proud to lead an
              institution dedicated to empowering students with the knowledge and skills
              needed to succeed in the digital age. With a career spanning several decades,
              I've mentored thousands of students and remain committed to making quality
              computer education accessible to all, fostering creativity and innovation.
            </motion.p>
            ...
            <motion.span
              ...
              className="text-base sm:text-lg md:text-xl font-semibold text-orange-300"
            >
              — Aditya Pandey
            </motion.span>
          </div>
      ...
      {/* Faculty Section */}
      <div className="bg-gradient-to-br from-orange-100 to-yellow-100 mx-4 sm:mx-8 
                      rounded-3xl p-4 sm:p-6 md:p-10 shadow-md">
        <motion.h2
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7 }}
          className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-orange-700 mb-6 sm:mb-10"
        >
          Meet Our Faculty
        </motion.h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {facultyMembers.map((member, index) => (
            <motion.div
              ...
              className="p-4 sm:p-6 bg-white rounded-2xl shadow-sm hover:shadow-lg 
                         flex flex-col items-center text-center transition-all duration-300"
            >
              <motion.div
                whileHover={{ scale: 1.05 }}
                transition={{ type: "spring", stiffness: 200 }}
                className="w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 border-2 border-orange-400 
                           rounded-2xl overflow-hidden mb-4"
              >
                <img
                  src={member.image}
                  alt={member.name}
                  className="w-full h-full object-cover bg-gray-100 rounded-2xl"
                />
              </motion.div>
              ...
              <p className="text-gray-600 text-sm sm:text-base mt-2 leading-relaxed px-2">
                {member.description}
              </p>
            </motion.div>
          ))}
